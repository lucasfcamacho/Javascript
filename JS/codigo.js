const modal = document.getElementById('modal');
        ...
        function mostrarUsuario(usuario) {
            ...
            btnModificar.addEventListener('click', e => {
                cambiarURL(`/modificar/${usuario.id}`, 'Modificar');
            });
        ...
        btnCrear.addEventListener('click', e => {
            cambiarURL('/crear', 'Crear');
        });
        
        btnCancelar.addEventListener('click', () => {
            cambiarURL('/home', 'Home');
        });
        
        btnGuardar.addEventListener('click', e => {
            const pathname = location.pathname.split('/');
            const metodo = pathname[1] === 'crear' ? 'post' : 'put';
            let url = 'https://jsonplaceholder.typicode.com/users'

            if (pathname[1] === 'modificar') {
                url += `/${pathname[2]}`;
            }
        ...
        function cambiarURL(url, title) {
            history.pushState(null, title, url);
            mostrarOcultar();
        }

        window.addEventListener('popstate', mostrarOcultar);
